...
- A description of one or more output fields (e.g., the question's answer) that we will expect from the LM. The output fields may include logic to handle truncation and noise in the predictions to ensure clean and accurate results.
...
You can optionally provide a `prefix` and/or `desc` key for each input or output field to refine or constrain the behavior of modules using your signature. Additionally, you can implement logic to truncate and clean the output fields to handle any noise or unwanted input data included in the predictions. The description of the sub-task itself is specified as the docstring (i.e., `"""Write a simple..."""`).
