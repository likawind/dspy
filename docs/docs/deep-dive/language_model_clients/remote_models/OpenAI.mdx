...
```python
gpt3_5 = dspy.OpenAI(model='gpt-3.5')
```
...
```python
dspy.configure(lm=gpt3_5)

#Example DSPy CoT QA program
qa = dspy.ChainOfThought('question -> answer')

response = qa(question="What is the capital of Paris?") #Prompted to gpt3_5
...
```python
response = gpt3_5(prompt='What is the capital of Paris?')
print(response)
```
