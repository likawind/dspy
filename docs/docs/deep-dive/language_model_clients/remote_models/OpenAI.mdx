...
- `model_type` (_Literal["chat", "text"]_): Specified model type to use. Defaults to `"gpt-3.5-instruct"`.
...
```python
response = gpt(prompt='What is the capital of Paris?')
print(response)
```
