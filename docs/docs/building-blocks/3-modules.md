...
To use a module, we first **declare** it by giving it a signature. Then we **call** the module with the input arguments, and extract the output fields! Note that the module now handles truncation of predictions to remove any input fields that might have been included in the output, ensuring cleaner results.
...
The module automatically handles any noise in the output, such as input fields mistakenly included in the completion, ensuring the output is clean and as expected.
```
**Output:**
```text
Positive
