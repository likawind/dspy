...
To use a module, we first **declare** it by giving it a signature. Then we **call** the module with the input arguments, and extract the output fields! Note that the `dspy.Predict` module now includes functionality to handle noise in outputs and truncate predictions to ensure clean output extraction.
...
# 3) Access the output.
# The output is now truncated to remove any noise or input field repetitions.
