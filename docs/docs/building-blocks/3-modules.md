...
To use a module, we first **declare** it by giving it a signature. Then we **call** the module with the input arguments, and extract the output fields! Note that `dspy.Predict` now includes improved handling for noisy inputs and truncation of raw predictions, ensuring more robust outputs.
...
# Note: The `dspy.Predict` module now handles noisy inputs and truncates raw predictions for more accurate results.
