...
Note that the `extract` function now includes logic to truncate raw predictions, which can help improve the accuracy of the predicted answers by removing any input fields that were mistakenly included in the completion.
